---
name: Vapory Development Tutorial
category: 
---

The purpose of this page is to serve as an introduction to the basics of Vapory that you will need to understand from a development standpoint, in order to produce contracts and decentralized applications. For a general introduction to Vapory, see [the white paper](https://github.com/vaporyco/wiki/wiki/White-Paper), and for a full technical spec see the [yellow](http://gavwood.com/Paper.pdf) papers, although those are not prerequisites for this page; that is to say, this page is meant as an alternative introduction to Vapory specifically targeted toward application developers.
...
![img](https://raw.githubusercontent.com/vaporybuilders/GitBook/master/en/vitalik-diagrams/contract_relationship.png)
...
![img](https://raw.githubusercontent.com/vaporybuilders/GitBook/master/en/vitalik-diagrams/contract_relationship2.png?1)
...
Fortunately, you do not have to program in low-level assembly; a high-level language exists, especially designed for writing contracts, known as [Solidity](https://github.com/vaporyco/wiki/wiki/Solidity) exists to make it much easier for you to write contracts (there are several others, too, including [LLL](https://github.com/vaporyco/cpp-vapory/wiki/LLL-PoC-5), [Serpent](https://github.com/vaporyco/wiki/wiki/Serpent) and [Mutan](https://github.com/vaporyco/go-vapory/wiki/Mutan-0.2), which you may find easier to learn or use depending on your experience). Any code you write in these languages gets compiled into EVM, and to create the contracts you send the transaction containing the EVM bytecode.
...
A contract by itself is a powerful thing, but it is not a complete Đapp. A Đapp, rather, is defined as a combination of a contract and a graphical interface for using that contract (note: this is only true for now; future versions of Vapory will include whisper, a protocol for allowing nodes in a Đapp to send direct peer-to-peer messages to each other without the blockchain). Right now, the interface is implemented as an HTML/CSS/JS webpage, with a special Javascript API in the form of the `eth` object for working with the Vapory blockchain. The key parts of the Javascript API are as follows:
...
You do not need any special source file or library to use the `eth` object; however, your Đapp will only work when opened in an Vapory client, not a regular web browser. For an example of the Javascript API being used in practice, see [the source code of this webpage](http://gavwood.com/gavcoin.html).

### Fine Points To Keep Track Of

See [https://github.com/vaporyco/wiki/wiki/Subtleties](https://github.com/vaporyco/wiki/wiki/Subtleties)